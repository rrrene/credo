defmodule Credo.Code.Token do
  ...
  # Elixir >= 1.20.0 sigil syntax
  def position(
        {:sigil, {line_no, col_start, {_line_no_end, _col_end}}, _, atom_or_charlist, _list,
         _number, _binary}
      ) do
    position_tuple_for_quoted_string(atom_or_charlist, line_no, col_start)
  end
  ...
  defp convert_to_col_end(
         _,
         _,
         {:sigil, {line_no, col_start, _end_position_or_nil}, _, list, _, _}
       )
       when is_list(list) do
    Enum.reduce(list, {line_no, col_start, nil}, &reduce_to_col_end/2)
  end
  ...
  defp convert_to_col_end(
         _,
         _,
         {:sigil, {line_no, col_start, _end_position_or_nil}, _, value, _, _}
       ) do
    {line_no, to_col_end(col_start, value), nil}
  end
