defmodule Credo.Check.Design.AliasUsage do
  ...
  defp find_issues({:@, _, _}, issues, _issue_meta, _excluded_namespaces, _excluded_lastnames, _aliases, _mod_deps) do
    {nil, issues}
  end
  # Ignore multi alias call
  defp find_issues({:., _, [{:__aliases__, _, _}, :{}]} = ast, issues, _issue_meta, _excluded_namespaces, _excluded_lastnames, _aliases, _mod_deps) do
    {ast, issues}
  end
  ...
  defp find_issues(ast, issues, _source_file, _excluded_namespaces, _excluded_lastnames, _aliases, _module_deps) do
    {ast, issues}
  end
  ...
  defp conflicting_alias?(alias_name, mod_list, last_name) do
    full_name = mod_list |> Credo.Code.Name.full_name
