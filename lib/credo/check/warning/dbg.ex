defmodule Credo.Check.Warning.Dbg do
  use Credo.Check,
    id: "EX5026",
    base_priority: :high,
    elixir_version: ">= 1.14.0-dev",
    param_defaults: [allow_captures: false],
    explanations: [
      check: """
      Calls to dbg/0 and dbg/2 should mostly be used during debugging sessions.

      This check warns about those calls, because they probably have been committed
      in error.
      """,
      params: [
        allow_captures: "Allow using a capture, e.g. `&dbg/1`."
      ]
    ],
    managed_traversal: :ast

  def handle_walk({:@, _, [{:dbg, _, _}]}, ctx) do
    {nil, ctx}
  end

  def handle_walk({:dbg, meta, []} = ast, ctx) do
    {ast, put_issue(ctx, issue_for(ctx, meta, "dbg"))}
  end

  def handle_walk({:dbg, meta, [_single_param]} = ast, ctx) do
    {ast, put_issue(ctx, issue_for(ctx, meta, "dbg"))}
  end

  def handle_walk({:dbg, meta, [_first_param, _second_param]} = ast, ctx) do
    {ast, put_issue(ctx, issue_for(ctx, meta, "dbg"))}
  end

  def handle_walk(
        {{:., _, [{:__aliases__, meta, [:"Elixir", :Kernel]}, :dbg]}, _, _args} = ast,
        ctx
      ) do
    {ast, put_issue(ctx, issue_for(ctx, meta, "Elixir.Kernel.dbg"))}
  end

  def handle_walk({{:., _, [{:__aliases__, meta, [:Kernel]}, :dbg]}, _, _args} = ast, ctx) do
    {ast, put_issue(ctx, issue_for(ctx, meta, "Kernel.dbg"))}
  end

  def handle_walk(
        {:&, _, [{:/, _, [{:dbg, _meta, _}, _arity]}]} = ast,
        %{params: %{allow_captures: true}} = ctx
      ) do
    {ast, ctx}
  end

  def handle_walk({:&, _, [{:/, _, [{:dbg, meta, _}, _arity]}]} = ast, ctx) do
    {ast, put_issue(ctx, issue_for(ctx, meta, "dbg"))}
  end

  def handle_walk({:|>, _, [_, {:dbg, meta, nil}]} = ast, ctx) do
    {ast, put_issue(ctx, issue_for(ctx, meta, "dbg"))}
  end

  def handle_walk(ast, ctx) do
    {ast, ctx}
  end

  defp issue_for(issue_meta, meta, trigger) do
    format_issue(
      issue_meta,
      message: "There should be no calls to `dbg/1`.",
      trigger: trigger,
      line_no: meta[:line],
      column: meta[:column]
    )
  end
end
