defmodule Credo.Check.LintAttributeTest do
  ...
  defp assert_value(source, expected) do
    ...
    assert expected == lint_attribute.value,
           "Expected #{inspect(expected)}, got #{inspect(lint_attribute.value)}"
  end

  test "it should return " do
    assert_value("@lint false", false)
  end
  ...
  test "it should work for a Regex" do
    assert_value("@lint {~r/Refactor/, false}", [{~r/Refactor/, false}])
  end
