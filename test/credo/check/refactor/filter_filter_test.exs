defmodule Credo.Check.Refactor.FilterFilterTest do
  use Credo.Test.Case
  ...
  test "it should report a violation /2" do
    ...
    defmodule Credo.Sample.Module do
      def some_function(p1, p2, p3, p4, p5, p6) do
        Enum.filter(Enum.filter([:a, :b, :c], &String.contains?(&1, "x")), &String.contains?(&1, "a"))
      end
    end
    """
    |> to_source_file
  ...
  test "it should report a violation /6" do
    ...
    defmodule Credo.Sample.Module do
      def some_function(p1, p2, p3, p4, p5) do
        Enum.filter([:a, :b, :c] |> Enum.filter(&String.contains?(&1, "x")), &String.contains?(&1, "a"))
      end
    end
    """
    |> to_source_file
  ...
  test "reports chains of Map.filter" do
    """
    defmodule Credo.Sample.Module do
      def some_function(p1, p2, p3, p4, p5) do
        %{"a" => 1, "b" => 2, "c" => 3}
  ...
  test "reports chains of Keyword.filter" do
    """
    defmodule Credo.Sample.Module do
      def some_function(p1, p2, p3, p4, p5) do
        [a: 1, b: 2, c: 3]
